...
**Follow-up (same day, after a self-critique pass flagged retrieval recall as the weakest part of the system):** extended `eval/retrieval_ablation.py` to a third configuration -- `BAAI/bge-small-en-v1.5`, a stronger, retrieval-tuned embedding model, with its recommended query-side instruction prefix. Result was, again, genuinely mixed rather than a clean win: BGE gets a much better **mean rank** (35.5 vs. 48.9 for the ablated MiniLM config) -- it is far less prone to catastrophically burying a relevant facet (e.g. `Intelligence Quotient (IQ)` ranks 139th under BGE vs. 330th under MiniLM) -- but it does **not** improve Recall@8 (45% vs. 50% for ablated MiniLM) or Recall@20 (60% vs. 65%), the cutoffs this system actually operates at. The ablated MiniLM configuration remains the best-supported choice of the three at n=20. Full numbers and the exact per-facet comparison: `eval/retrieval_ablation_report.md`. Production index was **not** swapped to BGE on the strength of this result -- swapping would mean re-running the full ~40min LLM scoring benchmark to keep `eval/report.md` consistent with whichever index actually generated it, and the recall-cutoff metrics that matter for this system's actual operating point don't support the swap yet. This is reported as a real negative-ish result, not reframed as a win because a bigger number (mean rank) happened to move.

---

## 6. Disclosing retrieval recall as its own number, not just inside a code comment

**Problem:** The benchmark (`eval/run_eval.py`) uses `force_facet_ids` to guarantee every reference facet reaches the scorer, so its 90% agreement figure measures the scoring stage, not the full retrieve-then-score pipeline end to end. This was already documented in a code comment, but a reviewer would have to notice that comment *and* separately find the ablation report *and* connect the two to realize organic retrieval alone only surfaces 45% of reference facets at the benchmark's `top_k=8`. That's an unreasonable amount of cross-referencing to expect from a reader -- in practice, the headline "90% agreement" number could be read as stronger evidence about the full pipeline than it actually is.

**Options considered:**
- **Leave it as a code comment.** Technically disclosed, defensible as "not hidden," but not something a reader would actually find without specifically going looking for it.
- **Compute and report organic retrieval recall as its own labeled section in `eval/report.md`, directly under the headline agreement/hallucination numbers.** Costs a small amount of extra computation per benchmark run (one additional `retrieve()` call per conversation, no LLM involved) and a few lines of report generation.

**Choice:** The second option -- implemented in `eval/run_eval.py`'s `compute_retrieval_recall()`, which measures what organic retrieval would have found *before* any force-adding happens, and a "Retrieval recall (unassisted)" section that is now the third thing `eval/report.md` reports, right after agreement and hallucination counts. A per-row "retrieved organically?" column was also added to the row-by-row table.

**Trade-off:** This makes the benchmark's headline metrics look less uniformly clean at a glance -- the report now leads with a caveat instead of just the good numbers. Accepted deliberately: a reviewer who reads `eval/report.md` top to bottom now cannot come away thinking retrieval was fully validated when it wasn't, without needing to already know to look for the catch. Making a weakness harder to miss, rather than technically-disclosed-but-easy-to-miss, is the actual goal of a prompt log and decisions doc, not a hit to take reluctantly.
