...
## Scope of this guarantee — an honest limit (found, fixed, and re-verified)

The taxonomy gate blocks hallucination on facets whose *category* is fundamentally unscorable from conversation (medical/clinical/cognitive). It does **not** — and cannot, by design — prevent the LLM from misjudging a facet that genuinely *is* conversation-observable. The first full benchmark run found exactly this kind of failure on `C03`: given a self-contradictory statement ("I trust people completely... actually no, I don't trust a single person"), the model scored `Trust in others` at 1/5 with 0.95 confidence, reasoning from only the *second* half of the contradiction, instead of recognizing the contradiction itself as a reason to abstain. That was a real failure on a real personality-trait facet the taxonomy correctly leaves in the LLM's hands — a different failure category from the three cases above (ambiguity-resolution, not a category-error hallucination), needing a different fix (prompt-level handling of contradictory evidence, not a taxonomy change).

It's fixed now: an abstract prompt rule alone didn't change the model's behavior (verified and reported as a negative result, not silently dropped); a concrete worked example in the system prompt did. Re-running the full benchmark confirmed **0/20 hallucinations** post-fix, with the `C03` row now correctly abstaining. Full diagnosis, both fix attempts, and both verification steps are in `DEBUGGING.md` #6 — including *why* the first attempt likely failed (tied to the 7B-model trade-off already recorded in `DECISIONS.md` #3).

A second, still-open failure remains in the current benchmark: `C04` (quoted speech) has the system scoring the speaker's `Risktaking` off a trait explicitly attributed to their sister. This is left unfixed and reported as-is in `eval/report.md`, rather than everything in this write-up being retrofitted to look clean — see README "what I'd improve with another day" for the candidate fix.
